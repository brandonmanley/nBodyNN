...
int main(int argc, char* argv[]) {
	...
	string fileString = outputDir + "brutus"+tostr(batchNum)+"_"+tostr(fileNum)+"_"+tostr(numberOfBodies);
	...
	int totalNumThreads = OMP_NUM_THREADS;

	for(int i = 0; i < totalNumThreads; i++){
		std::ofstream fB;
		fB.open(fileString+"_"+tostr(i)+".csv", ios_base::app);
	...
	#pragma omp parallel for default(private) firstprivate(numBits, pmax, t_end, fileNum, events)
	for(int iEvent=0; iEvent < events.size(); ++iEvent){

		int threadNum = omp_get_thread_num();
		string thread_fileString = fileString + "_" + tostr(threadNum) + ".csv";
		...
		*/ 
		...
		while(t<t_end){
			...
			eventString += to_string(fileNum) + "," + tostr(iEvent*10000+count) + ",";
		...
		std::ofstream fB; 
		fB.open(thread_fileString, ios_base::app);
		fB << eventString << endl;
		fB.close();
	}
