...
np.savetxt(workDir+"data/nbody/5body/X_train_5body.csv", X_train, delimiter=",")
np.savetxt(workDir+"data/nbody/5body/y_train_5body.csv", y_train, delimiter=",")
np.savetxt(workDir+"data/nbody/5body/X_test_5body.csv", X_test, delimiter=",")
np.savetxt(workDir+"data/nbody/5body/y_test_5body.csv", X_test, delimiter=",")
...
def kfold_network(X, y, hidden_nodes,n_layers,activation='relu',optimizer='adam'):
    ...
    network.save_weights(workDir + 'work/nbody/weights5/model_init.h5')
    
    #early stopping
    patienceCount = 20
    callbacks = [EarlyStopping(monitor='val_loss', patience=patienceCount),
                 ModelCheckpoint(filepath=workDir+'work/nbody/weights5/best_model_split'+str(numSplits)+'_nlayers'+str(n_layers)+'_nhidden'+str(hidden_nodes)+'.h5', monitor='val_loss', save_best_only=True)]
    ...
    for train_index, val_index in skf.split(X, y):
        ...
        network.load_weights(workDir + 'work/nbody/weights5/model_init.h5')
        ...
        network.save(workDir + 'work/nbody/weights5/trained_model_split'+str(numSplits)+'_nhidden'+str(hidden_nodes)+'.h5')

        plt.clf()
        plt.plot(history.history['mae'])
        plt.plot(history.history['val_mae'])
        plt.savefig(workDir + 'work/nbody/plots5/layers'+str(n_layers)+'_nodes'+str(hidden_nodes)+'_split'+str(numSplits)+'_mae.png')
...
np.savetxt(workDir+"data/nbody/5body/mae_map.csv", mae_map, delimiter=",")
np.savetxt(workDir+"data/nbody/5body/loss_map.csv", loss_map, delimiter=",")
...
np.savetxt(workDir+"data/nbody/5body/results.csv", array, delimiter=",")
