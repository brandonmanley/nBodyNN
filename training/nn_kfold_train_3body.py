...
#Import data
dataPath = workDir + "data/nbody/3body/"
nBodies = 3
dataCols = ["eventID","finalFile"]
...
X1 = df.as_matrix(columns=dataCols+i_col)
y1 = df.as_matrix(columns=o_col)

X1 = X1.astype('float64')
y1 = y1.astype('float64')

X_train,X_test,y_train,y_test = train_test_split(X1,y1, test_size=0.02, random_state=42)
print(X_train.shape, y_train.shape)
print(X_test.shape,y_test.shape)

trainMap = X_train[:,0:2]
testMap = X_test[:,0:2]
print(trainMap)
...
np.savetxt(workDir+"data/nbody/3body/trainMap3body.csv", trainMap, delimiter=",")
np.savetxt(workDir+"data/nbody/3body/testMap3body.csv", testMap, delimiter=",")
...
def kfold_network(X, y, hidden_nodes,n_layers,activation='relu',optimizer='adam'):
    ...
    callbacks = [EarlyStopping(monitor='val_loss', patience=patienceCount),
                 ModelCheckpoint(filepath=workDir+'work/nbody/weights3/best_model_split'+str(numSplits)+'_nlayers'+str(n_layers)+'_nhidden'+str(hidden_nodes)+'.h5', monitor='val_loss', save_best_only=True)]
    ...
    for train_index, val_index in skf.split(X, y):
        ...
        network.load_weights(workDir + 'work/nbody/weights3/model_init.h5')
        ...
        network.save(workDir + 'work/nbody/weights3/trained_model_split'+str(numSplits)+'_nhidden'+str(hidden_nodes)+'.h5')

        plt.clf()
        plt.plot(history.history['mae'])
        plt.plot(history.history['val_mae'])
        plt.savefig(workDir + 'work/nbody/plots3/layers'+str(n_layers)+'_nodes'+str(hidden_nodes)+'_split'+str(numSplits)+'_mae.png')
...
np.savetxt(workDir+"data/nbody/3body/mae_map.csv", mae_map, delimiter=",")
np.savetxt(workDir+"data/nbody/3body/loss_map.csv", loss_map, delimiter=",")
...
np.savetxt(workDir+"data/nbody/3body/results.csv", array, delimiter=",")
